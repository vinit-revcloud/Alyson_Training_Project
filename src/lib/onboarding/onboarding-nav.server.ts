...
export async function markSectionVisitedInDb(input: {
  userId: string;
  courseId: string;
  classId: string;
  sectionId: string;
  ...
  try {
    ...
    await client.query(
      ...
      [input.userId, input.sectionId, input.courseId, input.classId],
    );

    await client.query(
      `UPDATE profiles SET last_learn_course_id = $2, last_learn_section_id = $3 WHERE user_id = $1`,
      [input.userId, input.courseId, input.sectionId],
    );
